...
void depth_cb(const sensor_msgs::Image::ConstPtr &msg)
{
    ...
    cv_ptr_depth->image.convertTo(depth, CV_32FC1, 0.001);
    depth_update = true;
}

void info_cb(const sensor_msgs::CameraInfo::ConstPtr &msg)
{
    camera_info = *msg;
}
...
{   
    ...
    while(ros::ok()){ 
        ...
        depth_intrins.fx = camera_info.K[0];
        depth_intrins.fy = camera_info.K[4];
        depth_intrins.height = height;
        depth_intrins.width = width;
        ...
        // Mat color = cv_ptr_color->image;
        if (color_update && depth_update)
        {
            color_ = color;
                ...
                {   
                    ...
                    for (int j = -range; j < range+1; j++)
                    {
                        ...
                        {   
                            ...
                            {
                                float depth_search = depth.at<float>(out_corners[i].x + k, out_corners[i].y + j);
                            ...
                            {
                                float depth_search_in = depth.at<float>(in_corners[i].x + k, in_corners[i].y + j);
                    ...
                    // std::cout << "out_corner_depth: " << out_corner_depth << std::endl;
                    // std::cout << "in_corner_depth: " << in_corner_depth << std::endl;
